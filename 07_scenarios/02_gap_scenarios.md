...
-> 写 D_init / Q_entry -> 实际成交
-> 用关键结构证据变化更新 E -> Q_hold -> A
```
...
| 当前 gap 状态 | M / K 路由 | C 与 T | D_init / E 边界 |
| --- | --- | --- | --- |
| Opening gap 获得同方向跟进、回踩守住旧边界 | M4，并观察新趋势结构；离开旧区域下注 K2，既有趋势恢复下注 K1。 | 检查下一现实阻力、gap 边界和旧区域；等待 T2 follow-through、回踩守住或 T1 恢复。 | Invalidation 是新区间不再被接受；stop 使用完整突破/回踩结构。Gap 保留加强 E2，但不能单独确认。 |
| Opening gap 或突破形成的 gap 被进入，价格回旧区域并有反向跟进 | M4 失败并回到原基础状态或 M3；只回公平区域观察 K3。 | Gap reversal 只是早期事件；等待收回旧区域后的 failed entry、二次信号等 T3。 | Stop 在失败测试结构外；PT1 是旧公平区域中的第一现实目标。回补本身不足以确认 E3。 |
...
| 候选 measuring gap 仍开放且延续获得跟进 | 保持原 K1/K2，不生成新计划。 | 把 gap 投射与其他支撑阻力一起排序；它可以是 PT1、PT2 或仅为路径参考。 | D_init 冻结端点；只有最终形成 measured move 后才在复盘中确认名称。 |
| 成熟趋势末端出现异常大 gap / climax bars | 添加 M6，但不自动 K4。 | 先观察获利了结、小反转、结构破坏和旧极值测试；未出现前不逆势。 | 若只预期回到公平区域是 K3；只有控制权转移和反向 swing 空间成立才是 K4。 |
| Gap 状态未决、双方都无跟进 | 保留当前 M，或进入 E6。 | 不用“迟早回补”或“必然延续”选择方向；等待新信息。 | 空仓继续等待；持仓按真实目标、stop 和 Q_hold 管理。 |
...
6. 使用保守 entry、stop fill 和退出价后，Q_entry 是否仍合格？

## 成交后的更新

对使用了 gap 证据的计划，每次 D_live 都要把其状态或作用变化并入关键结构证据。Gap 仍开放不能代替 E2 与 Q_hold；gap 被进入或关闭也不能代替 E3。真正影响动作的是旧区域是否被重新接受、同/反方向 follow-through、剩余目标与 active stop，以及原命题是否仍成立。

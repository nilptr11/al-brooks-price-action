...
1. 先判断基础状态更接近趋势还是交易区间；若是趋势，再区分已形成方向控制的 breakout phase（spike）或 channel，并另外记录 breakout mode、高潮式推进或转换过程。真实标签看不清时保持不确定，只采用[逐层 fallback](strategy/how_to_choose.md#状态不清时的逐层-fallback)限制风险，不能把它当成策略许可。
2. 进入开盘或尾盘窗口时先检查时段覆盖，再按主要交易 premise 进入既有方向延续、新价格接受、双向平衡、方向未决或控制削弱／转移五条路径中的一条，只选择一项与当前机会完全相符的策略。
3. 在下单前一次写清 signal 或其他参照事件、图表触发、撤单、保护止损、目标、数量、四类风险、剩余持有时间、最晚入场、强制退出、管理和失效条件；账户成交或模拟成交随后按各自事实来源追加。
4. 按[执行手册](execution/execution_manual.md)处理订单、成交、所选环境确认的仓位、保护和异常。

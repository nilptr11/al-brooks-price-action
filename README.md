...
“执行权威”不等于“Brooks 官方原文”。执行手册包含仓库自己的 M/K/C/T/D/Q/P/E/A 路由、人的订单/成交边界和保守执行政策；G1–G3 是 `D_init` 内的管理配置，不是独立状态层。官方资料是证据权威，用来审计和修正这些政策。若学习文档与执行手册冲突，执行时以执行手册为准，同时在审计矩阵中记录并校准差异。
...
- `M/K/C/T/D/Q/P/E/A`、`D_init/D_live`、`Q_entry/Q_hold`、`G1–G3`、`PT1/PT2`、`MaxLoss`、`PlannedLoss`、`OpenRisk` 等内部简称只属于执行手册；训练、审计和研究文档可以明确地作为当前流程派生物引用。

...
| 你现在要做什么 | 从哪里开始 |
| --- | --- |
| 理解市场周期、背景、形态和交易原理 | [核心框架](core/README.md) |
...
1. 先判断基础状态更接近趋势还是交易区间；若是趋势，再区分突破或通道，并记录是否正在形成双向等待或趋势反转过程。看不清时保持未分类。
...
| [reference](reference/README.md) | 保存术语边界、正式来源和核心概念覆盖关系 |

核心理论不直接发出交易指令；策略页不重新解释基础概念；执行层不替交易者选择策略；参考资料不规定入场动作。同一条规则只在一个位置完整说明，其他地方只链接过去。

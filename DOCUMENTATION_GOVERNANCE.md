...
>
> 本文件只规范仓库结构、文档职责和维护流程，不定义交易动作。交易执行内容仅以 [`EXECUTION_MANUAL.md`](EXECUTION_MANUAL.md) 为准；[`EXECUTION_RECORD_TEMPLATE.md`](EXECUTION_RECORD_TEMPLATE.md) 只是派生记录工具。
...
- `EXECUTION_RECORD_TEMPLATE.md` 只把执行手册要求的信息排成可复制的批次配置和单笔交易版式，不定义准入、动作或例外。
- `training/` 保存连续回放流程、按需观察提示、逐根观察和复盘材料；它只派生使用学习与执行内容。
- `reference/` 保存稳定术语、官方来源登记和审计状态。
- 根目录文件负责全仓入口、唯一执行权威、唯一派生执行版式和文档治理，不在根目录堆放专题正文。
...
`Normative` 表示执行权威；`Governance / Normative` 只表示维护规则必须遵守。两者不得混用。`Execution / Derived`、`Training / Derived`、`Non-normative` 和 `Navigation` 文档即使包含空白字段、示例、摘要或引用，也不能覆盖其上游所有者。
...
1. 执行动作、字段含义或准入条件冲突，以 `EXECUTION_MANUAL.md` 为准；派生记录模板必须随之修正。
...
- `EXECUTION_RECORD_TEMPLATE.md` 只能依赖当前执行手册，把其要求排成空白版式；不能定义准入、阈值、动作、例外或字段语义。
...
- 核心执行流程和高级执行附录都使用可直接朗读的自然语言标题与问题；可复制的空白执行版式只在 `EXECUTION_RECORD_TEMPLATE.md` 排版。不得使用旧的单字母加数字阶段码、英文目标或损失字段标签，也不得用单字母链条建立隐藏流程。
...
| `EXECUTION_MANUAL.md` 的核心流程或高级附录 | `EXECUTION_RECORD_TEMPLATE.md`、`training/`、`reference/audit_matrix.md`；只有概念本身改变时才改 `learning/` |

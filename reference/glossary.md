...
预定目标先于 protective stop 到达。先获得 scalper's profit 只能说明尚未构成 strict failure，不自动等于 success；方向短暂走对也不等于成功。
...
官方 glossary 中，fade 是逆当前趋势交易，例如卖出预期会失败的牛突破；countertrend 是逆当前 Always In 方向的交易或 setup，两者高度重叠。本仓库说“区间边缘 fade”时，特指押注一次尝试失败并回到旧区域；交易区间没有清晰 Always In 方向时，它不一定构成 countertrend。
...
本仓库的描述性教学语言：触发后的力度或跟进弱于预期。它削弱 setup，但单独不足以确认 failure。官方 glossary 的 `Disappointment Bar` 更窄，特指 Buy The Close 牛趋势中的熊 K 线或 Sell The Close 熊趋势中的牛 K 线。
...
紧通道中至少三腿或三次 surge 构成的 climactic wedge。走势常给人加速、抛物线式推进的印象，但官方公开材料没有把“每一腿必须更陡”设为最低条件；它也不自动意味着 major reversal。
...
官方 abbreviations 把 triangle 定义为至少五次反转的 trading range，并以“两次 higher low 加一次 lower high”或镜像结构描述；它处于 breakout mode。收敛外观常见，但不能把任意三点收缩都直接计作完整 triangle；expanding triangle 是相关但独立的区间变体。
...
Scaling in 是分批增加仓位；scaling out 是分批退出。本仓库只允许事前绑定数量、价格和完整风险的方案；这是保守执行政策，不是术语的官方最低定义。
...
本仓库训练层对日内主要结构的归并。日型会随新信息改变，不应在开盘时被当成固定标签；跨市场时段倾向需要具体样本验证。

...
>
> 本表帮助快速读懂正文，不直接形成交易计划。仓库订单与记录操作见[执行手册](../EXECUTION_MANUAL.md#7-仓库订单与记录操作)；来源与审计入口见[参考与来源](README.md)。

只收录核心流程反复使用、容易混淆或最低定义会改变判断的词。形态细节由 [`learning/04_patterns/`](../learning/04_patterns/README.md) 解释，低频词直接在首次出现处说明。
...
交易数学。比较成功概率与回报、失败概率与风险，判断交易是否具有正的数学期望。佣金和滑点另行影响真实净结果。
...
预定目标先于 protective stop 到达。先获得 scalper's profit 不自动等于完整目标已经成功；方向短暂走对也不等于 success。
...
一段运动走得过快过远，并且市场已经反向进入 trading range 或相反 trend。反转出现前可以描述为高潮式推进或 possible climax，但不能只因价格“太高、太低、太快”就确认 climax。
...
Fade 是逆当前 trend 交易，例如卖出预期会失败的 bull breakout；countertrend 是逆当前 Always In 方向的交易或 setup，两者高度重叠。在 trading range 边缘交易 failed breakout 时，市场可能没有清楚的 Always In 方向，因此不一定属于 countertrend。
...
Buy The Close bull trend 中的 bear bar，或 Sell The Close bear trend 中的 bull bar。日常所说的 entry disappointment 只是价格行为弱于预期，单独不足以确认 failure。
...
已经实际入场、尚未获得目标或至少 scalper's profit、当前处于开放亏损，并很可能被迫退出的一方。已经退出者可记录为“曾被困并被迫退出”，不再属于当前 `trapped in a trade` 状态。
...
至少一端严格越过、另一端相等或越过前一根的 K 线：high > 前高且 low ≤ 前低，或 low < 前低且 high ≥ 前高。它表示当根双向扩张。
...
H1/H2 对回调中的第一次/第二次向上触发尝试计数；L1/L2 对第一次/第二次向下触发尝试计数。Brooks glossary 使用严格 `above` / `below`，而 *10 Best Price Action Trading Patterns* 的图例写作 `at or above` / `at or below`，所以相等高低点的边界在公开材料中并不完全一致。计数的交易含义来自 context、回调次数和触发质量，不能只靠是否相等决定。
...
紧通道中至少三腿或三次 surge 构成的 climactic wedge。走势常给人加速、抛物线式推进的印象，但不要求“每一腿必须更陡”；它也不自动意味着 major reversal。
...
Triangle 是至少五次反转的 trading range，可由“两次 higher low 加一次 lower high”或镜像结构形成，并处于 breakout mode。收敛外观常见，但不能把任意三点收缩都直接计作完整 triangle；expanding triangle 是相关但独立的区间变体。
...
当日第一根 K 线开盘越过上一时段最后一根 K 线，或越过整个上一时段范围；向上为 gap up，向下为 gap down。Gap 方向本身不决定当日趋势方向。
...
两个参照 K 线的实体不重叠形成的空间，影线可以重叠；参照对象可以是突破点 K 线与数根之后的回测 K 线，不要求相邻。
...
Scaling in 是分批增加仓位；scaling out 是分批退出。Scale-in 会改变总仓位、average price、stop risk 和 Trader's Equation。
...
描述一个交易时段主要由 trend、trading range 或 reversal 主导。日型会随新信息改变，不应在开盘时被当成固定预测；跨市场时段倾向需要具体样本验证。

...
基础状态先分 trend 与 trading range；trend 再观察当前主要是获接受的 breakout phase（spike）还是 channel，实务上可看成 `accepted breakout phase / spike -> tight channel -> broad channel -> trading range` 的连续谱。Breakout mode、climactic move 与 transition evidence 是可叠加的跨层信息，不与两种基础状态并列。
...
Likely 或 probably 表示概率向一侧倾斜，不表示确定；risky 提醒成功概率、风险或现实回报可能不足。实际判断必须回到同一组 entry、protective stop、target 和管理，而不能只凭词语强弱。
...
Breakout event 是当前 K 线高点或低点越过重要旧价位；接受证据未知时先视为 breakout attempt。强收盘、follow-through 和没有快速收回支持突破已获接受；获接受的突破进一步形成持续方向控制后，才可称为 trend 中的 breakout phase（spike）。
...
任一方向突破都应有获得 follow-through 的可能，不预设方向，也不保证第一次突破获得接受。它是可叠加的跨层候选状态，不是与 trend / trading range 并列的第三种基础状态，也不是已经发生的 breakout event 或 breakout phase。
...
派生自：[Core Definition：Climax / Transition](../core/01_market_cycle/04_climax_and_transition.md#定义)。
...
Support 和 resistance 是市场上下测试留下的反应区域，不是保证反转的精确线。Magnet 是可能吸引价格测试的目标区域；吸引不保证到达。
...
Stop entry 在价格向交易方向越过触发价后入场；buy stop 在当前价上方触发买入，sell stop 在当前价下方触发卖出；protective stop 则用于退出并保护持仓。二者用途不同，实际成交都可能因跳空或滑点偏离触发价。
...
Signal bar 是实际 entry bar 前、用于放置入场单的最后一根 K 线；没有真实入场时，更准确地称为 prospective signal bar（候选信号 K 线）。Entry bar 是订单实际成交所在的 K 线。
...
与当前 trend 或 leg 方向相反的 trend bar。反向测试影线和靠近新方向极值的收盘是常见加强项；外观良好仍不表示 Context 支持反转。
...
成熟趋势末端仍像普通 continuation pattern 的整理，实时只能标记为 candidate final flag。只有随后顺势突破或恢复尝试失败，并出现反向强度或退出压力，才支持 confirmed final-flag interpretation。

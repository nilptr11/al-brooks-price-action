...
>
> 本表只统一跨章节反复使用、混淆后会改变判断的术语边界，不直接形成交易计划。仓库订单与记录操作见[执行手册](../EXECUTION_MANUAL.md#7-仓库订单与记录操作)；来源与审计入口见[参考与来源](README.md)。

使用时直接搜索英文词。只需确认最低定义时查本表；需要识别线索、例图、stop、target 或管理方式时，按[学习路径](../LEARNING_PATH.md#按需专题)进入对应专题。未收录的普通词和专题词在首次出现处或专题正文解释，本表不扩写成第二本教材。

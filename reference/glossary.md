...
>
> 本术语表只做快速解释，不定义交易动作。执行规则以当前[执行手册](../EXECUTION_MANUAL.md)为准；官方来源和核对状态见 [`reference/official_sources.md`](official_sources.md) 与 [`reference/audit_matrix.md`](audit_matrix.md)。

本术语表按学习流程分类，而不是按字母排序。定义尽量保持简短；复杂边界应放入学习文档或审计记录，不在这里形成第二套执行规则。若官方最低定义与本仓库的执行准入不同，本表先写官方定义，只说明“术语定义不等于执行许可”并指向执行手册；不复制内部流程字段、编号或具体批准阈值，也不能用内部政策改写官方术语。
...
交易设置。官方 glossary 的最低定义是：由 context 和一根或多根 K 线构成、可作为放置入场单依据的 pattern；订单实际成交后，setup 的最后一根才成为 signal bar。Setup 不等于完整风险计划；可批准交易还需按执行手册完成失效、protective stop、仓位、目标、管理和 Trader's Equation。
...
交易区间。官方 glossary 的最低结构甚至可以从一根范围被前一根大幅重叠的 K 线开始，表示短期横向、双方都未控制；发展后的 trading range 会反复双向尝试并围绕公平区域交易。最低术语定义不等于可交易准入；具体执行边界另见执行手册。
...
均线缺口 K 线。整根 K 线没有触及均线。在强趋势首次深回调中形成该结构后，市场通常还会测试原趋势极值；它是 setup 背景，不是机械入场信号。

### Second Moving Average Gap Bar Setup

第二个均线缺口 K 线 setup。出现第一次 moving-average gap bar 后，若随后朝均线的反转未触及均线、价格又继续远离均线，则下一次再次朝均线的反转是第二个 setup。

### 20 Moving Average Gap Bars

约二十根均线缺口 K 线。连续二十根或更多 K 线未触及均线；市场最终首次触及均线时，通常形成再次测试原趋势极值的 setup。
...
保护性止损。把交易命题的失效事实落实为结构外的有限损失订单；它不同于 stop entry，也不必机械等于 signal bar 另一端。

### Invalidation

命题失效事实，也常简称“无效点”。它是足以否定当前交易命题的可观察市场事实；结构锚点和 protective stop 据此确定，但三者不是同一事物。
...
逆势 scalp。认为原趋势仍会继续，但短期小回调即将出现，试图逆势抓小利润。官方 glossary 明确警告这通常是错误、应避免；是否允许及其数学门槛另见执行手册。
...
失败。交易者尚未先获得原目标或至少 scalper's profit，protective stop 却先被触发，常导致反向运动。

### Failed Entry

失败入场。实际 entry 后尚未先获得原目标或至少 scalper's profit，protective stop 却先到；单纯弱跟进或普通 pullback 还不能确认失败。
...
失败二次入场。H2 / L2 或 second entry 实际触发后，没有先获得原目标或至少 scalper's profit，protective stop 却先到；立即失望不等于已经确认失败。
...
被困交易者。已经实际入场、尚未先获得原目标或至少 scalper's profit、处于开放亏损，并在反向运动中很可能被迫或已经实际退出的一方。
...
缺口。最窄定义是两根价格 K 线之间出现空间；Brooks 的实务用法还会比较 K 线高低开收、旧 swing、突破点、均线或趋势线等支撑阻力对象。它可能是 opening gap，也可能是日内结构性空间。

### Gap Up / Gap Down

向上 / 向下开盘缺口。当日第一根的开盘价高于昨日最后一根或整个昨日范围的高点，或低于相应低点。Opening gap 成立不要求当日第一根的完整高低范围始终留在缺口外。

### Gap Close

缺口回补。价格重新进入选定 gap 的旧侧边界；具体比较对象必须事先写明。它说明原强度证据减弱，但没有回到旧区域和反向跟进时，不自动构成趋势反转。

### Gap Reversal

缺口反转。官方最低几何事件是当前 K 线向 gap 方向越过前一根 K 线至少一跳；例如 gap up 后第二根跌破第一根低点一跳。它不表示价格已到达真正的 gap 边界、gap 已回补、反向已被接受或可以直接入场。

### Body Gap

实体缺口。回测时影线可以重叠，但回调 K 线的实体仍未与突破点 K 线的实体重叠。官方 glossary 用突破后约 5—10 根的回调举例，这是典型案例而不是固定时间阈值；它也不是泛指任意相邻两根实体之间的小空隙。
...
突破形成的 gap。强突破后，突破收盘、旧区间边界或随后价格之间形成空间；它强调市场可能接受新区间，是否保留仍由回调和跟进验证。这里是仓库的描述性归并，不宣称 `breakout gap` 是官方 glossary 的独立最低术语。
...
失败缺口。市场开在新区域后无法继续，并回到原区域，使追随 gap 方向的交易者被困。这里描述的是结果路径；官方 glossary 的独立最低事件名称是 `gap reversal`。
...
以小利润快速退出、通常不准备穿越正常 pullback 的交易，常见于 trading range 或双边市场。官方 glossary 警告：若潜在回报小于风险，需要多数交易者难以达到的极高胜率；除非极有经验，潜在回报至少应不低于风险。仓库采用何种更保守准入政策只在执行手册中定义。
...
Ten Bars, Two Legs correction。反转 swing 常见的时间—腿数经验预期，大约十根 K 线并形成两腿；它不是 correction 成立的硬阈值，也不是任意两根反向 K 线。
...
歧义术语。Brooks 的公开 Ask Al / webinar 管理语境会把 `Actual Risk` 与 `Initial Risk` 对照，前者可能在入场后小于后者；官方 web glossary 又在 `risk` 条目中用小写 `actual risk` 指滑点导致的更大实际损失。两者不能混成“交易结束后的真实盈亏”。在公开来源未给出统一计算定义前，执行手册使用自身的无歧义风险字段，不把 `Actual Risk` 直接用于下单或仓位公式。

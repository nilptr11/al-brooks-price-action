...
策略的全局生命周期、时段覆盖、单候选、计划结束与重入由[如何选择并锁定一项策略](../strategy/how_to_choose.md)规定；路由进入一条路径后，路径内分类归对应 README，具体资格与参数归策略页。本页只映射概念角色，不产生另一套路由规则。

## 五种角色

| 角色 | 含义 |
| --- | --- |
| 市场背景 | 帮助选择价格行为路径，本身不直接触发交易 |
...
| 仓库推导 | 把多个 Brooks 概念组合成单一候选，划分策略页、选择目标锚点、规定策略之间的归类与切换 | 这是可追溯的人工整理，不是新增的 Brooks 定义或通用概率结论 |
...
五条路径是 Strategy 为选择单一计划建立的仓库路由，不是 Brooks 官方市场状态分类。路径按当前计划的主要 premise 区分；开盘与尾盘只提供覆盖约束，不与五条路径并列。

| 路径 | Core 证据怎样进入 | 策略去向 |
| --- | --- | --- |
| 既有方向延续 | 趋势控制已经建立，主要摆动仍完整，回调后原方向恢复 | [趋势回调策略](../strategy/trend/README.md) |
| 新价格接受 | 事前边界被越过，收盘、跟进、测试守住或 failed failure 支持外部价格 | [突破延续策略](../strategy/breakout/README.md) |
| 双向平衡 | 成熟交易区间继续主导，边缘拒绝或失败突破支持回到内部价值 | [交易区间策略](../strategy/range/README.md) |
| 方向未决 | Context 尚不能排除任一方向，且双侧计划都完整、有利 | [双向等待策略](../strategy/breakout_mode/README.md) |
| 控制削弱或转移 | 末端与逆向压力只支持修正，或 MTR 证据链已经支持主要控制转移 | [逆向修正与 MTR](../strategy/reversal/README.md) |
| 时段覆盖层 | 开盘定价、固定窗口、尾盘时间风险和 session magnet 加严候选、目标或退出边界 | [开盘与尾盘](../strategy/session/README.md) |

同一段价格可以同时包含 breakout、pullback、wedge 或 opening 等描述；归类看所选计划依赖什么，而不是看名称出现了几个。突破后的第一次边界测试仍可属于新价格接受路径；普通趋势回调则属于既有方向延续。逆向修正和 MTR 共用控制削弱／转移路径，但属于两个不同子家族，不能互相借用目标或确认强度。
...
| Trend 与 channel | 市场背景 | 根据主要 premise 进入既有方向延续、新价格接受或控制削弱／转移路径；趋势和通道本身不是入场信号 |

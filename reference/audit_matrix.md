...
| 自然语言核心流程与高级附录边界 | `EXECUTION_MANUAL.md`“核心五问”、“高级执行附录” | `training/README.md`、`training/02_bar_by_bar_templates.md`、`training/03_review_checklist.md` | — | 内部执行政策 | 内部执行政策；复杂订单仅在实际相关时进入同一权威文件的附录，不另建流程 |
| 账户、单笔与时段风险范围 | `EXECUTION_MANUAL.md`“使用边界”、“一份核心交易计划” | `README.md`、`LEARNING_PATH.md` | — | 范围声明 | 内部治理；未另行设定上位风险边界时只用于模拟与复盘，核心学习不构成实盘许可 |
...
| Entry、premise 失效与 protective stop | 执行：`EXECUTION_MANUAL.md`“Protective stop、目标和仓位”；教学：`learning/03_order_flow/01_stop_entry_vs_protective_stop.md` | `learning/06_trade_management/00_trade_plan.md` | `SRC-GLOSSARY`、`SRC-STOP-ORDERS`、`SRC-RISK-113`、`SRC-GOOD-TRADE-2017` | 官方 stop 依据 + 本仓库四层执行拆分 | 内部执行政策；合理 price-action stop、多个可行 stop 与远 stop 配小仓的官方底座已核对，四层拆分及不得放宽属于仓库政策 |

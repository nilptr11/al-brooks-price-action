...
本目录提炼 Al Brooks Price Action 的核心理念：市场如何在 trend、trading range、breakout 和 channel 之间转换，交易者如何结合 context、接受与订单逻辑和交易数学构造并持续复核一笔交易。
...
目录编号只负责稳定分组，首次阅读按概念依赖推进：

1. 先读[方法与读图主线](00_method/00_al_brooks_thesis.md)和 [Trader's Equation](00_method/01_probability_risk_reward.md)，建立整套判断的约束。
2. 依次理解 [Market Cycle](01_market_cycle/00_market_cycle.md)、[Context](02_context/00_context_location_control.md)和[两类 Stop](03_acceptance_and_order_logic/00_stop_entry_vs_protective_stop.md)。
3. 再读[形态语言](04_patterns/README.md)、[Setup](05_setups/README.md)和[交易计划与管理](06_trade_plan_and_management/README.md)。
4. 候选触发和成交后，用[接受、失望与失败证据](03_acceptance_and_order_logic/01_acceptance_and_failure.md)持续更新原判断。

各专题仍会相互引用；这条路径只解决第一次阅读的前置依赖，不把动态决策过程改写成单向流程。
...
| Market Cycle | [市场周期](01_market_cycle/00_market_cycle.md) | 定义 trend / trading range 与 breakout / channel 层级 |
...
| [`05_setups/`](05_setups/README.md) | 比较交易命题、入场依据、特有失效边界、stop 锚点和结果预期 | 固化最终仓位、订单价格和成交后完整管理 |
| [`06_trade_plan_and_management/`](06_trade_plan_and_management/README.md) | 把 Setup、风险、目标、仓位和管理组成同一方案 | 重新定义上游形态或市场状态 |

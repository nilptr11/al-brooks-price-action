...
边缘回归如何形成交易命题见[交易区间 Fade Setup](../05_setups/03_trading_range_fade.md)，限价单占优时的订单行为见 [Limit Order Market](../03_order_flow/02_limit_order_market.md)。

## 区间突破

区间中的突破应先观察，而不是立刻相信。强收盘、突破强度和 follow-through 是市场开始接受新价格的主要证据。并非每次成功突破都会先回踩；若随后确实出现回踩，守住突破点是额外的接受证据，没有回踩本身不构成突破失败。

如果突破后缺乏跟进并快速回到区间内，突破尝试可能形成 failed breakout，追随者也可能被困。若这一反向尝试又失败，则可能形成 failed failure，并引发更强运动。这里的 failure 描述价格运动，实际 trade failure 的结果边界见[接受、失望与失败证据](../03_order_flow/00_acceptance_and_failure.md#disappointmentpremise-变化和-trade-failure)。

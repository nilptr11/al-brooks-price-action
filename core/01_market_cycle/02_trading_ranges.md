...
这里的分母是“区间内的一次突破尝试”，不是“区间最终是否突破”。成熟区间最终会突破，最终方向常接近 50/50；约 `80%` 的尝试失败也不适用于 breakout mode，课程对后者的成功/失败先验更接近 50/50。三个数字描述不同事件，不得互换。
...
相关来源见 [`reference/official_sources.md`](../../reference/official_sources.md) 中的 `SRC-GLOSSARY`、`SRC-TRADING-RANGES`、`SRC-10-PATTERNS`、`SRC-COURSE-01-36`（课程 18A p1455、18B p1476、18F p1593、19A p1619）。

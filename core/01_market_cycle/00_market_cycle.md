...
这些都不是静态标签。形成方向控制的 breakout phase（spike）可以进入 tight channel，再扩展为 broad channel 或 trading range；trading range 边界可以出现 breakout event / attempt，失败后仍维持区间，获得接受并形成方向控制后才可能把基础状态更新为 trend。

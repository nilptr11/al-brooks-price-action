...
Trap 不能只凭走势看起来不利就事后命名。至少需要有证据表明：

- 一方很可能已经实际触发入场；
- 原目标或至少 scalper's profit 尚未先实现；
- 已入场一方处于开放亏损；
- 反向价格行为正在威胁其退出边界或已经迫使退出。

Entry disappointment、premise 变化、trade failure 和 trapped traders 的严格边界见[接受、失望与失败证据](00_acceptance_and_failure.md)。确认反方被困也只生成新的候选；反向交易仍需自己的 Context、Setup 和 entry。
...
完整交易仍由 [Trade Plan](../06_trade_management/00_trade_plan.md) 检查，不能从“二次”这个计数直接推导 reward/risk。

...
这里的 protective stop 是保护实际持仓的退出 stop，scalper's profit 是典型 scalp 目标；它们只界定结果，不提供新的 entry 依据。本节必须结合 [Stop Entry 和 Protective Stop](01_stop_entry_vs_protective_stop.md) 的完整定义理解，不能混用两类 stop。

本文单独使用 **trade failure** 表示实际交易的结果。`failed breakout`、`failed H2` 或“旧极值测试失败”描述的是某次价格运动未达到预期，不自动表示某位交易者已经触及 protective stop。使用 failure 时应写清失败的对象。
...
Entry disappointment 和 premise 变化描述不同阶段的价格行为，两者都不自动等于 trade failure。正常 swing pullback 不能仅因令人失望就归为 failed entry；在 protective stop 前提前退出时，分别记录当时 premise 判断和实际价格结果。

...
- Limit probe 或事前设计的 scale-in 可能需要允许更深的边缘外测试，因此 stop 更宽、仓位更小；这一 stop 和全部计划数量的总风险应在第一笔 entry 前一起评估。

区间边界本身不是自动止损线。Stop 必须位于能够证明“旧区间仍在主导”这一命题错误的结构外，并给正常的边缘 overshoot 留出空间。
...
相关来源见 [`reference/official_sources.md`](../../reference/official_sources.md) 中的 `SRC-MANUAL`、`SRC-TRADING-RANGES`、`SRC-STOP-ORDERS`、`SRC-SCALE-IN-TRENDS` 与 `SRC-STRONG-LEGS-2016`。

...
这里的 gap 比传统“隔夜跳空”更广：任何两个支撑阻力对象之间的空间都可能被称为 gap，对象可以是 K 线的高低开收、旧 swing、突破点、均线或趋势线。Gap 的核心含义是某段价格没有形成充分的双边交易，因此它可能提供强度、接受/拒绝、支撑阻力和目标线索。
...
Gap 是 context，不是独立入场信号。交易仍要由位置、触发、合理 stop、现实 target 和 Trader's Equation 共同支持。
...
这种区分很重要：同一个开放空间在趋势建立阶段可能是 strength，在成熟趋势末端可能只是 potential exhaustion；后续行为出现前不能用结果名称替代当时判断。
...
Gap reversal 的最低定义是当前 K 线向 gap 方向越过前一根 K 线至少一跳；例如 gap up 后第二根跌破第一根低点一跳。它是拒绝路径的初始证据，不保证价格已经触及真正的 gap 区域，更不是交易许可；开盘案例中更可靠的反转通常还要结合昨日高低点、二次信号、signal bar 和后续跟进。

## 突破形成的空间、Body Gap 和 Micro Measuring Gap

Breakout 收盘越过旧高低点或其他重要边界时，突破更强；收盘与旧边界之间的空间经常成为 measuring-gap 候选，并可在回测时承担支撑阻力作用。

Body gap 常在突破后的回测中出现：影线已经重叠，但实体仍不重叠。突破后约 5—10 根的回调是一个典型案例，不是形成 body gap 的固定时间阈值。Body gap 比完整价格 gap 弱，但仍是接受和强度证据。

Micro measuring gap 比较强趋势 K 线前后两根 K 线的完整范围。牛例中，后一根低点不低于前一根高点；熊例镜像，两条边界的中点可以作为量度参考。它经常支持至少第二腿或 measured move，但不提供确定概率，也不免除对路径障碍和剩余时间的检查。

这些 gap 后续被进入或回补，只表示原强度证据削弱。没有回到旧区域和反向跟进时，不能仅凭“gap 消失”宣布趋势反转。

## Moving-Average Gap Bar

Moving-average gap bar 是整根 K 线不触及均线。常见的两种语境是：

- 强趋势首次深回调穿过均线以后，牛趋势的 moving-average gap bar 整根位于均线下方，即该 K 线最高价低于当根均线值；熊趋势镜像，整根位于均线上方，即最低价高于当根均线值。随后市场通常还会测试原趋势极值。
- 连续约 20 根或更多 K 线未触及均线后，首次触及均线常形成测试原趋势极值的 setup。

出现第一次 moving-average gap bar 后，如果第一次朝均线的反转未能触及均线，价格随后再次远离均线，那么第二次朝均线的反转称 second moving-average gap bar setup。牛趋势例中，价格位于均线下方，第一次向上反转的最高价仍低于均线，随后价格再次下跌远离均线；下一次向上反转就是第二次机会。熊趋势完全镜像。

这些都是趋势背景和 setup 线索，不是机械在均线挂单的理由；仍要检查趋势是否强、当前触发是否成立，以及正确 stop 是否可承受。

## Measuring Gap 与 Exhaustion Gap

Measuring gap 是最终带来 measured move 的 breakout。多数目标首先按突破前交易区间或形态高度投射；在特定图表中，也可以把 gap 的中点或靠近运动起点的一侧边缘当作量度参考。实时只能记录“候选 measuring gap”，并持续检查 gap 是否开放、突破是否被接受以及目标前是否存在更近障碍。
...
- **市场周期**：趋势建立阶段的 gap 常支持 breakout 或 tight channel；成熟趋势末端的大 gap 需要同时观察是否形成 climax。
- **支撑阻力**：Gap 边界、突破点和旧价格区域可能成为回测位置或 trapped traders 的退出位置。
- **目标**：Gap 可以构成回补目标、路径障碍或 measured-move 参考，但不能保证到达。
- **止损**：Gap 边界只有在当前 setup 中确实构成合理 price-action stop 时才使用，不能机械使用 gap 中点。

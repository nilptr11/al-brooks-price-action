...
总体已经收敛到同一条主链条：market cycle、控制权、位置、触发、跟进、失败、被困交易者、交易数学和管理。真正需要防止的不是重复定义，而是把不同层级的概念压成同义词。

最容易混淆的地方有三类：

1. 把市场状态、订单环境和交易方式混在一起。例如 trading range、limit order market 和 fade 相关，但不是同义词。
2. 把触发、结果和参与者混在一起。例如 failed entry 是触发失败，trapped traders 是失败后的仓位压力。
3. 把结构预期和管理模式混在一起。例如 TBTL 说明时间和两腿预期，scalp / swing 说明如何管理仓位。
...
Surprise 和 inertia 不能替代 follow-through。Surprise 说明行为超出预期，inertia 说明市场可能再次测试该方向；它们仍然要看后续是否接受新价格。

### Control 和 Location

Control 回答谁更强，location 回答是否值得在这里交易。

强控制权可以提高顺势概率，但如果入场位置太差、止损太远或目标太近，Trader's Equation 仍可能不合格。

### Failure 和 Trapped Traders

Failure 说明原方向没有成功，trapped traders 说明失败后有多少人会被迫行动。

不是所有失败都值得交易。只有反向行为足够强，并且价格有合理目标时，被困交易者才可能成为有效推动力。

Failed failure 不是普通 failure 的同义词。它表示先交易“失败”的一方也失败了，连续两批交易者被迫退出，因此经常比单次失败更有推动力。

### TBTL 和 Swing / Scalp

TBTL 是结构和时间预期，swing / scalp 是管理方式。

一笔交易可以用 TBTL 作为 swing 的初始预期，但不能因为看到 TBTL 语言就自动做 swing。反过来，区间 scalp 即使出现两腿结构，也不代表应该临场扩大成 swing。

## 学习压缩规则

1. 遇到新术语，先问它属于主链条的哪一层。
2. 如果它只描述形状，先归入 pattern，不直接交易。
3. 如果它描述触发后的结果，重点看 follow-through、failure 和 trapped traders。
4. 如果它描述位置或目标，必须和风险回报一起看。
5. 如果它无法说明止损和管理方式，就还不是交易计划。

...
| 层级 | 作用 | 编号 |
| --- | --- | --- |
| M 市场状态 | 判断当前 market cycle 和控制权 | M1 趋势 / 紧通道；M2 通道 / 弱趋势；M3 交易区间；M4 突破模式；M5 高潮 / 转换 |
| E 确认事件 | 判断触发、跟进、失败和被困交易者 | E1 回调失败并顺势恢复；E2 突破被接受；E3 突破失败；E4 failed failure；E5 结构破坏 + 回探失败；E6 无跟进 / 混乱 |
| P 交易计划 | 决定是否交易以及交易什么 | P1 趋势延续；P2 突破延续；P3 区间边缘 fade；P4 反转 / TBTL；P0 No-trade |
...
stateDiagram-v2
  state "P1 趋势延续" as P1
  state "P2 突破延续" as P2
  state "P3 区间边缘 fade" as P3
  state "P4 反转 / TBTL" as P4
...
├─ M2 通道 / 弱趋势
│  ├─ E1 好位置顺势恢复 -> P1 趋势延续
│  ├─ E3 通道边缘突破失败 -> P3 区间边缘 fade
│  ├─ 双边交易增加 -> M3 交易区间
│  └─ E6 结构不清或空间不足 -> P0 No-trade
├─ M3 交易区间
│  ├─ 区间中部或空间不足 -> P0 No-trade
│  ├─ E3 边缘突破失败 -> P3 区间边缘 fade
│  ├─ E2 强突破并有跟进 -> P2 突破延续
│  └─ E4 failed failure -> P2 突破延续
├─ M4 突破模式 / 压缩
│  ├─ 突破前 -> G0 等待
│  ├─ E2 突破被接受 -> P2 突破延续
│  ├─ E3 首次突破失败 -> P3 区间边缘 fade
│  └─ E6 反复失败 / Barbwire -> P0 No-trade
└─ M5 高潮 / 转换
   ├─ 顺势仍有跟进 -> P1 / P2
   ├─ E5 结构破坏 + 回探失败 + 反向跟进 -> P4 反转 / TBTL
   └─ E6 反向无跟进 -> M3 交易区间化
```
...
条件：价格接近上沿或下沿，突破或测试缺乏跟进，并出现反向强度。
...
第一目标通常是区间中轴；只有反向强度足够、目标空间足够时，才看另一侧边缘。Measured move 通常不是区间 fade 的第一目标，因为 fade 的前提是市场还没有接受离开区间。

无效点在失败突破结构外。如果突破强且有跟进，fade 分支失效。
...
这时不再机械 fade，策略切换到突破延续。此时 measured move 重新变重要，常用区间高度从突破点或突破区域投射。

### E4 / P2. Failed failure 事件

条件：先出现 failed breakout，反向 fade 交易者入场；随后反向交易失败，价格再次强势突破。

这类分支常产生更强运动，因为先后两批交易者被迫退出。Protective stop 应放在 failed failure 结构外，目标按突破延续处理，通常优先看区间 measured move 和下一支撑阻力。

失效转移：

- 边缘 fade 无反向跟进：回到 M3 或 P0。
- 突破有跟进并守住：转 P2。
- 强突破后又快速回到区间：回到 P3 或 P0。

## P2. 突破延续计划

分支命题：市场离开旧价格区域后被接受，错过者追随，站错者退出，价格开始寻找新的公平区域。
...
突破追随者可能被困，分支切换为区间 fade、开盘反转、failed gap 或 MTR 观察。
...
- 突破强且有跟进：切到 P2 突破延续。
- 首次突破失败：切到 P3 区间 fade 或 trapped traders。
...
### P4.3 Wedge / Final flag 失败

Wedge reversal、parabolic wedge 和 final flag 的共同点是：原趋势方最后一次推进或整理后缺乏跟进，顺势交易者被困。
...
| 场景 | 优先观察 | 编号映射 |
| --- | --- | --- |
| Trend from the open | 早期强突破、浅回调、反方失败 | P1 / P2 |
| Opening range breakout | 突破开盘区间后是否有跟进 | P2；失败则 P3 / P4 |
| Opening reversal | 早期运动进入关键位置后失败 | P3 / P4 |
| Gap accepted | 缺口方向有跟进，回补失败 | P2 / P1 |
...
| Wedge | Wedge pullback 可顺势 | 边缘三推可支持 fade | 趋势后期可支持 reversal / TBTL |
| Double top / bottom | Double top bear flag / double bottom bull flag | 边缘第二次测试失败可 fade | 回探极值失败可成为 MTR 组件 |
...
- 区间 fade：失败突破的极值外侧，或区间边缘测试失败结构外侧。
- 突破延续：突破失败位置外侧，或回踩突破点后形成的回踩结构外侧。
- 突破模式：突破前通常没有合格止损；等突破方向、回踩或失败结构出现后，再按 P2 或 P3 定位。
...
- 区间 fade 的第一目标通常不是量度目标，而是区间中轴；只有突破或 failed failure 被接受后，量度目标才重新变重要。
...
| P3 区间边缘 fade | 失败突破极值外、区间边缘结构外 | 区间中轴 | 反向强度足够且空间足够，才看另一侧边缘 | 优势来自边缘和失败，默认不是远目标交易 |
| E4 / P2 Failed failure | failed failure 结构外 | 区间 measured move、下一支撑阻力 | 按突破延续管理，快速回旧区则退出或反向评估 | 二阶失败可增强动能，但必须快速证明 |
| P2 突破延续 | 突破失败位置外、回踩结构外 | 区间高度、突破腿、开盘区间或缺口形成的 measured move；同时检查下一支撑阻力 | 突破后小回调趋势成立，才保留 swing 或跟踪目标 | 强突破概率高，但入场和止损通常更差 |
| M4 突破模式 | 突破前不预设；突破后按 P2，失败后按 P3 | 突破前无交易目标；突破后按 P2/P3 重新定目标 | 只有突破被接受，才用压缩区高度估 measured move | 突破前接近中性，优势在等待确认 |
| P4 反转 / TBTL | 回探失败极值外、反转结构失效位置外 | 均线、前 swing point、区间中轴 | 反向跟进强，才看 TBTL 或更大 swing | 早期概率偏低，需要更好目标空间或后续强突破 |
...
| P3 区间边缘 fade | 边缘 limit entry，或收回区间后的 stop entry | 失败突破极值外 | 区间中轴 | G1 scalp；强反向才看另一侧 |
| E4 / P2 Failed failure | 反向 fade 失败后的再突破 | failed failure 结构外 | 区间 measured move、下一支撑阻力 | 按 P2 管理 |
| P2 突破延续 | Stop entry，close entry，回踩守住再触发 | 突破失败位置或回踩结构外 | measured move、下一支撑阻力 | G3 swing；快速回旧区则退出或反向评估 |
| M4 突破模式 | 等待突破或失败确认 | 突破前不预设 | 突破前无目标；被接受后看压缩区 measured move | G0 等待；确认后切 P2/P3 |
| P4 反转 / TBTL | 回探失败后的反向触发 | 反转结构被否定的位置外 | 均线、前 swing point、区间中轴 | G4 TBTL / 保守 swing |
...
1. 当前 M 状态、E 事件或 P 计划有没有增强？
2. 如果没有增强，它降级到哪个 M 状态或 P 计划？
3. 是否已经触发 no-trade 条件？

常见转移规则：

- 趋势延续无跟进，先降级为通道或交易区间，不立刻假设大反转。
- 区间 fade 遇到强突破和跟进，立刻切到突破延续，不继续机械 fade。

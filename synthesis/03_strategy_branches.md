...
| 层级 | 作用 | 编号 |
| --- | --- | --- |
| M 市场状态 | 判断当前 market cycle | M1 趋势 / 紧通道；M2 通道 / 弱趋势；M3 交易区间；M4 突破模式；M5 高潮 / 转换 |
...
这里把 breakout 放在 E2 / P2，而不是单独列成稳定的 M 状态。原因是 breakout 更像市场离开旧区域的事件；只有被接受后，它才发展成趋势、通道或新的交易区间。M 层保留的是交易者在入场前更容易识别的背景状态。
...
1. M：先判断 market cycle。当前是趋势、通道、交易区间、突破模式，还是高潮 / 转换。
...
执行门槛：M3 和 M4 中的交易更依赖限价思维、宽止损、分批管理和快速降级。没有成熟的 limit order / scale-in 能力时，区间中部、紧区间和 barbwire 默认应更靠近 P0，而不是强行寻找 stop entry。
...
条件：先出现 failed breakout，反向回归交易者入场；随后反向交易失败，价格再次强势突破。
...
- 突破强且有跟进：切到 P2 突破延续。
- 首次突破失败：切到 P3 边缘失败 / 回归计划，或按 trapped traders 处理。
...
- P3 边缘失败 / 回归交易的第一目标通常不是量度目标，而是区间中轴；只有突破或 failed failure 被接受后，量度目标才重新变重要。
...
| E4 / P2 Failed failure | 反向回归交易失败后的再突破 | failed failure 结构外 | 区间 measured move、下一支撑阻力 | 按 P2 管理 |
...
- P3 回归交易遇到强突破和跟进，立刻切到突破延续，不继续机械回归。

...
本文件记录 `al-brooks-pattern-atlas.html` 的来源层级、定义边界和绘图验收规则。它不是新的交易系统；主定义仍由 `concept_index.md` 指向各主题文件。

## 来源层级

1. [Brooks Trading Course Glossary](https://www.brookstradingcourse.com/price-action-trading-terms-glossary/)：术语最低定义、K 线关系和缩写的首要依据。
2. [10 Best Price Action Trading Patterns](https://www.brookstradingcourse.com/price-action/10-best-price-action-trading-patterns/)：H1/H2、旗形、楔形、MTR 等面向交易者的概述；与 glossary 出现宽松措辞差异时，在图中保留 stop-entry 的严格触发语义。
3. [Distinguishing Strong Legs: Trading Ranges vs Trends](https://www.brookstradingcourse.com/wp-content/uploads/2016/05/Al-Brooks-Webinar-Distinguishing-Strong-Legs-TR-vs-Trends-May-3-2016.pdf)：强趋势、紧通道、宽通道和交易区间的视觉差异。
4. [Major Trend Reversals coaching session](https://www.brookstradingcourse.com/wp-content/uploads/2025/08/250830-Coaching-Session-3-MTRs.pdf)：MTR 的成熟趋势、趋势线破坏、旧极值测试和反向跟进。
5. [Opening Reversals](https://www.brookstradingcourse.com/wp-content/uploads/2017/10/171017-Futures.io-Presentation-Opening-Reversals-Al-Brooks.pdf) 与 [Patterns on the Open](https://www.brookstradingcourse.com/wp-content/uploads/2018/10/181030-futuresio-Patterns-on-the-Open-Al-Brooks.pdf)：opening reversal、trend from the open 和开盘后形态。
6. [Encyclopedia chart-pattern sampler](https://www.brookstradingcourse.com/price-action/brooks-encyclopedia-chart-patterns-sampler/) 和 Ask Al 文章用于补充语境，不覆盖 glossary 的最低定义。
...
| 9–13 | `pullback-pause-bar`, `ii-iii`, `ioi`, `oo`, `signal-entry-bar` | Glossary: Pullback Bar, Pause Bar, ii, ioi, oo, Signal/Entry Bar | pause 与 bar pullback 分离；iii 画满母 K + 三根 inside；Signal/Entry 按订单角色标记 | 已校准 |
...
| 18–21 | `spike`, `spike-channel`, `small-pullback-trend`, `micro-channel` | Glossary 对应条目 | spike 用连续大趋势 K；channel 阶段斜率和实体下降；小回调趋势回调浅；牛微通道每根低点不低于前低 | 已校准 |
| 22–26 | `stairs`, `shrinking-stairs`, `leg-two-legs`, `climax`, `overshoot` | Glossary 对应条目 | 新极值间距、两腿停顿、加速与通道线越界必须由实际摆动表达；不把 overshoot 自动画成反转 | 已校准 |
...
| 44–49 | `h1`, `h2`, `l1`, `l2`, `h2-bull-flag`, `l2-bear-flag` | Glossary；10 Best Patterns；Stop Orders manual | Atlas 按实际 stop-entry 触发画 H/L 计数；H2/L2 可像微双底/顶但不是必要外观 | 已校准 |
...
| 76–78 | `breakout-gap`, `measuring-gap`, `exhaustion-gap` | Glossary 对应条目 | 突破缺口区域持续开放；量度缺口投射等长运动；衰竭缺口是快速回补后的事后分类，区域在回补处终止 | 已校准 |
...
- Measuring Gap 的前后投射必须等长；Exhaustion Gap 的高亮区域在回补 K 处结束。

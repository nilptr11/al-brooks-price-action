...
2. 一边记录市场状态、位置和判断变化，一边在自然出现完整候选时用[执行记录模板](EXECUTION_RECORD_TEMPLATE.md)补齐 entry、失效、stop、第一目标、仓位和简单 Trader's Equation；计划合格才进入回放或模拟成交。
3. 在同一记录中练订单状态、成交保护和持仓管理，结束后只复盘一个主要流程问题；若使用真实资金，必须先满足执行手册规定的账户、单笔、时段和相关敞口边界。
